...
class ALU:
  ...
  def update(self, srcA, srcB, aluopcode):
    self.srcA = srcA
    self.srcB = srcB
...
def __init__():
  self.map = {}    
def update(self, opcode, rs, rt, rd, shamt, funct):
...
class MIPSProcessor:
  def __init__(self, program):
      ...
      self.data_memory = DataMemory(4294967296)  # 2^32 locations
      self.ALU = ALU()
  ...
  def decode_and_execute(self, instruction):
      ...
      self.controlUnit.update(opcode, rs, rt, rd, shamt, funct)
